...
The mean of the total number of steps taken each day is **`r totalStepsByDay.mean`** while its median is **`r totalStepsByDay.median`**.
...
The 5-minute interval containing the maximum number of steps is **`r avgStepsByInterval.maxInterval`**.
...
The total number of rows with missing values is **`r activity.numMissingRows`**.
...
The mean of the total number of steps taken each day, after imputing, is **`r imputedActivity.totalStepsByDay.mean`** while its median is **`r imputedActivity.totalStepsByDay.median`**. The mean is larger than before, while the median stays the same.
